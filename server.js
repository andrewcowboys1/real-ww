...
// Point Quo Settings → Webhooks to: https://YOUR-RAILWAY-URL/api/webhooks/quo
...
function extractCallInfo(body) {
  // Supports legacy (data.object) and beta (data.resource + data.context) formats
  ...
  const from = obj.from || ctx.senderIdentifier || ctx.from || body.from || '';
  const to = obj.to || (ctx.recipientIdentifiers && ctx.recipientIdentifiers[0]) || body.to || '';
  const status = obj.status || '';
  const callId = obj.id || body.id || '';
  const voicemail = obj.voicemail || null;
  return { type, direction, from, to, status, callId, voicemail, raw: body };
}
...
app.post('/api/webhooks/quo', (req, res) => {
  ...
  (async () => {
    try {
      ...
      const isIncoming = String(info.direction).toLowerCase() === 'incoming';
      const isSmsIn = /message\.received/i.test(info.type);
      ...
      const recent = data.callLog.find(c => c.callId && info.callId && c.callId === info.callId);
      if (recent) return;
      ...
      // Try Quo contacts for name + email
      ...
      if (existingLead) {
        ...
        if (existingLead.name === 'Unknown caller' && quoContact?.name) existingLead.name = quoContact.name;
      ...
      saveData(data);
      console.log(`[Quo webhook] ${sourceLabel} from ${lead.phone} email=${lead.email || 'n/a'} → ${lead.id} + ${wo.id}`);
    } catch (err) {
