...
app.patch('/api/work-orders/:id', (req, res) => {
  ...
  const body = req.body || {};
  // Don't let clients overwrite id / createdAt
  delete body.id;
  delete body.createdAt;
  Object.assign(wo, body, { updatedAt: new Date().toISOString() });
  if (body.assignedTo !== undefined) {
    const tech = data.users.find(u => u.id === body.assignedTo);
    wo.assignedName = tech ? tech.name : null;
    if (tech && tech.van) wo.assignedVan = tech.van;
  }
  if (body.status && body.status !== prevStatus) {
    const stamp = new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });
    wo.notes = (wo.notes ? wo.notes + '\n' : '') + `[${stamp}] Status → ${body.status.replace(/_/g, ' ')}`;
  }
  // Refresh calendar link when schedule/customer fields change
  wo.googleCalendarLink = buildGoogleCalendarLink(wo);
  saveData(data);
  res.json(wo);
});
