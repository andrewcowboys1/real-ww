...
async function adminLeads() {
  ...
  const content = `
    <div class="space-y-3">
      <button id="newLead" class="w-full btn-gold py-2.5 rounded-xl text-sm">+ Add Lead</button>
      ${leads.length===0?'<p class="text-center text-slate-400 py-8 text-sm">No leads yet</p>':
      leads.map(l => `
        <div class="border rounded-2xl p-4">
          ...
          <div class="mt-3 flex flex-wrap gap-2 items-center">
            ...
            ${l.status!=='converted'?`
              <button class="convert-lead-btn btn-navy text-xs px-3 py-1.5 rounded-lg" data-id="${l.id}">→ Job</button>
              <button class="convert-req-btn border text-xs px-3 py-1.5 rounded-lg" data-id="${l.id}">→ Request</button>
            `:''}
          </div>
        </div>`).join('')}
    </div>`;
  $('#app').innerHTML = shell('Leads', content, 'leads');
  bindNav();

  $('#newLead').onclick = async () => {
    const name = prompt('Name');
    if (!name) return;
    const phone = prompt('Phone') || '';
    const serviceInterest = prompt('Service interest (e.g. HVAC - Install)') || '';
    const source = prompt('Source (Google, Referral, Website…)') || 'Other';
    const notes = prompt('Notes') || '';
    const body = { name, phone, serviceInterest, source, notes, status: 'new' };
    try {
      if (useBackend) await api('/api/leads', { method:'POST', body: JSON.stringify(body) });
      else {
        const d = await getData();
        d.leads.unshift({ id:'LD-'+(d.nextLeadId++), ...body, email:'', address:'', createdAt:new Date().toISOString(), updatedAt:new Date().toISOString() });
        localSave(d);
      }
      toast('Lead added');
      adminLeads();
    ...
    sel.onchange = async () => {
      try {
        if (useBackend) await api('/api/leads/'+sel.dataset.id, { method:'PATCH', body: JSON.stringify({ status: sel.value }) });
          ...
          if (l) { l.status = sel.value; l.updatedAt = new Date().toISOString(); localSave(d); }
        }
  ...
  $$('.convert-lead-btn').forEach(btn => {
    btn.onclick = async () => {
      ...
      try {
        if (useBackend) {
          await api('/api/leads/'+btn.dataset.id+'/convert', {
            method:'POST', body: JSON.stringify({ to:'work_order', assignedTo: techId })
          });
        } else {
          const l = data.leads.find(x => x.id === btn.dataset.id);
          if (!l) return;
          const tech = data.users.find(u => u.id === techId);
          data.workOrders.unshift({
            id:'WO-'+(data.nextId++), leadId:l.id, customerName:l.name, customerPhone:l.phone,
            customerAddress:l.address, serviceType:l.serviceInterest||'Service', description:l.notes||'',
            assignedTo:techId, assignedName:tech?.name||null, status:'scheduled', scheduledDate:null,
            scheduledTime:'Anytime', notes:`From lead ${l.id}`, photos:[], createdAt:new Date().toISOString(), updatedAt:new Date().toISOString()
          });
          l.status = 'converted';
          localSave(data);
        }
  ...
  $$('.convert-req-btn').forEach(btn => {
    btn.onclick = async () => {
      try {
        if (useBackend) {
          await api('/api/leads/'+btn.dataset.id+'/convert', { method:'POST', body: JSON.stringify({ to:'request' }) });
        } else {
          const d = await getData();
          ...
          d.requests.unshift({
            ...
            customerAddress:l.address, serviceType:l.serviceInterest||'Service', description:l.notes||'',
            status:'pending', preferredDate:'', preferredTime:'Anytime', createdAt:new Date().toISOString(), photos:[]
          });
          l.status = 'converted';
          localSave(d);
        }
        toast('Converted to request');
        adminLeads();
      } catch(e) { toast(e.message,'error'); }
    };
  });
