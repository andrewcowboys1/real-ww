...
function timeWindowOptions(selected) {
  return TIME_WINDOWS.map(t => `<option value="${t}" ${selected===t?'selected':''}>${t}</option>`).join('');
}
...
function renderLogin() {
      ...
      <div class="flex-1 px-6 py-8 -mt-4">
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
          ...
          <div class="mt-5 pt-5 border-t border-slate-100 text-center space-y-2">
            ...
            <button id="showRegister" class="text-brand-800 font-semibold text-sm">Create customer account</button>
            <p class="text-sm text-slate-500 pt-2">Employee?</p>
            <button id="showStaffRegister" class="text-brand-800 font-semibold text-sm">Create employee account</button>
          </div>
  ...
  const ssr = $('#showStaffRegister');
  if (ssr) ssr.onclick = () => renderRegister('employee');
}

function renderRegister(kind='customer') {
  $('#app').innerHTML = `
    <div class="min-h-screen flex flex-col">
      <div class="bg-brand-800 text-white px-6 pt-10 pb-8">
        ...
        <h1 class="text-xl font-bold">${kind==='employee'?'Create Employee Account':'Create Customer Account'}</h1>
        <p class="text-slate-300 text-sm mt-1">${kind==='employee'?'For technicians & office staff':'Schedule service online'}</p>
      </div>
      <div class="px-6 py-6">
        <form id="regForm" class="space-y-3">
          ...
          ${kind==='employee'?`
          <div><label class="text-xs font-medium text-slate-500">Role</label>
            <select name="role" class="w-full border rounded-xl px-3 py-2.5 text-sm mt-1">
              <option value="technician">Technician</option>
              <option value="admin">Office / Admin</option>
            </select></div>
          <div><label class="text-xs font-medium text-slate-500">Staff signup code</label>
            <input name="staffCode" required class="w-full border rounded-xl px-3 py-2.5 text-sm mt-1" placeholder="Ask your manager" />
            <p class="text-[10px] text-slate-400 mt-1">Default code: CALLISON2026 (change in Railway env STAFF_SIGNUP_CODE)</p>
          </div>`:''}
  ...
  $('#regForm').onsubmit = async (e) => {
    ...
    if (kind === 'employee') {
      body.role = body.role || 'technician';
    } else {
      body.role = 'customer';
    ...
    try {
      if (useBackend) {
        ...
        if (kind === 'employee') {
          const code = (body.staffCode || '').trim();
          if (code !== 'CALLISON2026') throw new Error('Invalid staff signup code');
        }
        const nu = { id:'u'+Date.now(), name:body.name, email:body.email, password:body.password, phone:body.phone||'', address:body.address||'', role: body.role || 'customer' };
        data.users.push(nu);
        localSave(data);
      ...
      <form id="schedForm" class="space-y-3">
        ...
        <div>
          ...
          <select name="preferredTime" class="w-full border rounded-xl px-3 py-2.5 text-sm mt-1">
            ${timeWindowOptions('Morning (8am–12pm)')}
          </select>
        </div>
        <div>
...
function leadCardHtml(l, techs) {
  ...
  return `
    <div class="border rounded-2xl p-4 ${l.status==='lost'?'opacity-75 bg-slate-50':''}" data-lead-card="${l.id}">
      ...
      ${l.problem?`<div class="text-xs text-slate-700 mt-1 bg-amber-50 rounded-lg px-2 py-1"><strong>Problem:</strong> ${l.problem}</div>`:''}
      ${followNote}
      ${l.transcript?`<details class="mt-1"><summary class="text-[10px] text-brand-800 cursor-pointer">Call transcript</summary><pre class="text-[10px] text-slate-500 whitespace-pre-wrap max-h-32 overflow-auto mt-1">${l.transcript}</pre></details>`:''}
      ...
      <div id="convert-panel-${l.id}" class="hidden mt-3 p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
        ...
        <div class="conv-job-fields-${l.id} space-y-2">
          <select class="conv-tech w-full border rounded-lg text-xs px-2 py-1.5" data-id="${l.id}">
            ...
            ${techs.map(t => `<option value="${t.id}">${t.name}${t.van?' · '+t.van:''}</option>`).join('')}
          </select>
...
async function adminLeads(showLost=false) {
  ...
  const statusOrder = showLost ? ['lost'] : ['new', 'contacted', 'quoted', 'converted'];
  const statusLabels = { new:'New', contacted:'Contacted', quoted:'Quoted', converted:'Converted', lost:'Lost' };
  const sections = statusOrder.map(st => {
    const items = all.filter(l => l.status === st);
    if (!items.length && st !== 'new') return '';
    return `
      <div class="mt-2">
        <div class="flex items-center gap-2 mb-2">
          <span class="status-pill ${statusColor(st)}">${statusLabels[st]||st}</span>
          <span class="text-xs text-slate-400">${items.length}</span>
        </div>
        ${items.length ? items.map(l => leadCardHtml(l, techs)).join('') : '<p class="text-xs text-slate-400 mb-3">None</p>'}
      </div>`;
  }).join('');
  ...
  const content = `
    <div class="space-y-3">
      <div class="flex gap-2">
        <button id="tabActiveLeads" class="flex-1 py-2 rounded-xl text-sm font-medium ${!showLost?'btn-navy':'border text-slate-500'}">Pipeline (${active.length})</button>
      ...
      ${sections || '<p class="text-center text-slate-400 py-8 text-sm">No leads</p>'}
    </div>`;
...
function gcalLink(wo) {
  if (wo.googleCalendarLink) return wo.googleCalendarLink;
  const title = encodeURIComponent(`${wo.serviceType||'Service'} – ${wo.customerName||''} (${wo.id})`);
  const details = encodeURIComponent([wo.description||'', wo.customerPhone?`Phone: ${wo.customerPhone}`:'', wo.assignedName?`Tech: ${wo.assignedName}`:''].filter(Boolean).join('\n'));
  const location = encodeURIComponent(wo.customerAddress||'');
  let dates = '';
  if (wo.scheduledDate) {
    const d = String(wo.scheduledDate).replace(/-/g,'');
    if (/^\d{8}$/.test(d)) {
      const next = new Date(wo.scheduledDate+'T12:00:00'); next.setDate(next.getDate()+1);
  ...
  if (!dates) {
    ...
    dates = `${d}/${t2.toISOString().slice(0,10).replace(/-/g,'')}`;
  }
...
function woCardHtml(wo, techs) {
  return `
    <div class="border rounded-xl p-3 mb-2" data-wo-card="${wo.id}">
      <div class="flex justify-between items-start gap-2">
        <div>
          <div class="font-medium text-sm">${escAttr(wo.customerName)}</div>
          <div class="text-xs text-slate-500">${escAttr(wo.serviceType)} · ${escAttr(wo.assignedName)||'Unassigned'} · ${wo.id}</div>
          <div class="text-xs text-slate-400">${wo.scheduledDate||'No date'} ${escAttr(wo.scheduledTime||'')}</div>
          ${wo.customerAddress?`<div class="text-xs text-slate-400">${escAttr(wo.customerAddress)}</div>`:''}
          ${wo.customerPhone?`<div class="text-xs text-slate-400">${escAttr(wo.customerPhone)}</div>`:''}
        </div>
        <span class="status-pill ${statusColor(wo.status)}">${wo.status.replace(/_/g,' ')}</span>
      </div>
      ${wo.description?`<p class="text-xs text-slate-600 mt-1">${escAttr(wo.description)}</p>`:''}
      <div class="mt-2 flex flex-wrap gap-2 items-center">
        ${statusSelect(wo.status, wo.id)}
        <select class="assign-select border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}">
          <option value="">Reassign…</option>
          ${techs.map(t=>`<option value="${t.id}" ${t.id===wo.assignedTo?'selected':''}>${escAttr(t.name)}${t.van?' · '+escAttr(t.van):''}</option>`).join('')}
        </select>
        <button type="button" class="edit-wo-btn text-xs border border-brand-800 text-brand-800 px-2 py-1.5 rounded-lg" data-id="${wo.id}">Edit</button>
        <a href="${gcalLink(wo)}" target="_blank" rel="noopener" class="text-xs btn-navy px-2 py-1.5 rounded-lg">📅 Calendar</a>
      </div>
      <div id="edit-wo-${wo.id}" class="hidden mt-3 p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
        <div class="text-xs font-semibold text-slate-700">Edit work order ${wo.id}</div>
        <div class="grid grid-cols-2 gap-2">
          <div>
            <label class="text-[10px] text-slate-500">Customer name</label>
            <input class="wo-edit-name w-full border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}" value="${escAttr(wo.customerName)}" />
          </div>
          <div>
            <label class="text-[10px] text-slate-500">Phone</label>
            <input class="wo-edit-phone w-full border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}" value="${escAttr(wo.customerPhone||'')}" />
          </div>
        </div>
        <div>
          <label class="text-[10px] text-slate-500">Address</label>
          <input class="wo-edit-address w-full border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}" value="${escAttr(wo.customerAddress||'')}" />
        </div>
        <div>
          <label class="text-[10px] text-slate-500">Service type</label>
          <select class="wo-edit-service w-full border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}">
            ${['HVAC - Repair','HVAC - Maintenance','HVAC - Install','Electrical - Repair','Electrical - Panel Upgrade','Electrical - Install','Callback from phone','Emergency','Other'].map(s =>
              `<option value="${s}" ${wo.serviceType===s?'selected':''}>${s}</option>`).join('')}
          </select>
        </div>
        <div>
          <label class="text-[10px] text-slate-500">Description / problem</label>
          <textarea class="wo-edit-desc w-full border rounded-lg text-xs px-2 py-1.5" rows="2" data-id="${wo.id}">${escAttr(wo.description||'')}</textarea>
        </div>
        <div class="grid grid-cols-2 gap-2">
          <div>
            <label class="text-[10px] text-slate-500">Date</label>
            <input type="date" class="wo-edit-date w-full border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}" value="${wo.scheduledDate||''}" />
          </div>
          <div>
            <label class="text-[10px] text-slate-500">Time window</label>
            <select class="wo-edit-time w-full border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}">
              ${timeWindowOptions(wo.scheduledTime||'Anytime')}
            </select>
          </div>
        </div>
        <div>
          <label class="text-[10px] text-slate-500">Assign tech</label>
          <select class="wo-edit-tech w-full border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}">
            <option value="">Unassigned</option>
            ${techs.map(t=>`<option value="${t.id}" ${t.id===wo.assignedTo?'selected':''}>${escAttr(t.name)}${t.van?' · '+escAttr(t.van):''}</option>`).join('')}
          </select>
        </div>
        <div>
          ...
          <select class="wo-edit-priority w-full border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}">
            ${['low','normal','high'].map(p => `<option value="${p}" ${(wo.priority||'normal')===p?'selected':''}>${p}</option>`).join('')}
          </select>
        </div>
        <div>
          <label class="text-[10px] text-slate-500">Notes</label>
          <textarea class="wo-edit-notes w-full border rounded-lg text-xs px-2 py-1.5" rows="2" data-id="${wo.id}">${escAttr(wo.notes||'')}</textarea>
        </div>
        <div class="flex gap-2">
          <button type="button" class="save-wo-btn btn-gold flex-1 text-xs py-2 rounded-lg" data-id="${wo.id}">Save changes</button>
          <button type="button" class="cancel-edit-wo-btn border flex-1 text-xs py-2 rounded-lg" data-id="${wo.id}">Cancel</button>
        </div>
      </div>
...
async function adminOrders() {
  ...
  const techs = (data.users||[]).filter(u => u.role==='technician');
  const order = ['scheduled','on_the_way','in_progress','completed','cancelled'];
  const labels = { scheduled:'Scheduled', on_the_way:'On the way', in_progress:'In progress', completed:'Completed', cancelled:'Cancelled' };
  const sections = order.map(st => {
    const items = wos.filter(w => w.status === st);
    return `
      <div class="mt-3">
        <div class="flex items-center gap-2 mb-2">
          <span class="status-pill ${statusColor(st)}">${labels[st]||st}</span>
          <span class="text-xs text-slate-400">${items.length}</span>
        </div>
        ${items.length ? items.map(wo => woCardHtml(wo, techs)).join('') : '<p class="text-xs text-slate-400 mb-2">None</p>'}
      </div>`;
  }).join('');

  const content = `
    <div class="space-y-1">
      <p class="text-xs text-slate-500">Jobs by status. Use <strong>Edit</strong> to change details, or 📅 for Google Calendar.</p>
      ${wos.length===0?'<p class="text-sm text-slate-400 py-6 text-center">No work orders yet</p>':sections}
    </div>`;
  ...
  $$('.edit-wo-btn').forEach(btn => {
    btn.onclick = () => {
      const panel = $(`#edit-wo-${btn.dataset.id}`);
      if (panel) panel.classList.toggle('hidden');
    };
  });
  $$('.cancel-edit-wo-btn').forEach(btn => {
    btn.onclick = () => {
      const panel = $(`#edit-wo-${btn.dataset.id}`);
      if (panel) panel.classList.add('hidden');
    };
  });
  $$('.save-wo-btn').forEach(btn => {
    btn.onclick = async () => {
      const id = btn.dataset.id;
      const body = {
        customerName: $(`.wo-edit-name[data-id="${id}"]`)?.value?.trim() || '',
        customerPhone: $(`.wo-edit-phone[data-id="${id}"]`)?.value?.trim() || '',
        customerAddress: $(`.wo-edit-address[data-id="${id}"]`)?.value?.trim() || '',
        serviceType: $(`.wo-edit-service[data-id="${id}"]`)?.value || '',
        description: $(`.wo-edit-desc[data-id="${id}"]`)?.value || '',
        scheduledDate: $(`.wo-edit-date[data-id="${id}"]`)?.value || null,
        scheduledTime: $(`.wo-edit-time[data-id="${id}"]`)?.value || 'Anytime',
        assignedTo: $(`.wo-edit-tech[data-id="${id}"]`)?.value || null,
        priority: $(`.wo-edit-priority[data-id="${id}"]`)?.value || 'normal',
        notes: $(`.wo-edit-notes[data-id="${id}"]`)?.value || ''
      };
      btn.disabled = true;
      btn.textContent = 'Saving…';
      try {
        if (useBackend) {
          await api('/api/work-orders/'+id, { method:'PATCH', body: JSON.stringify(body) });
        } else {
          const d = await getData();
          const wo = d.workOrders.find(w => w.id === id);
          if (!wo) throw new Error('Not found');
          const tech = d.users.find(u => u.id === body.assignedTo);
          Object.assign(wo, body, {
