    ...
    <div class="border rounded-xl p-3 mb-2">
      <div class="flex justify-between items-start gap-2">
        <div>
          ...
          <div class="text-xs text-slate-400">${wo.scheduledDate||'No date'} ${wo.scheduledTime||''}</div>
          ${wo.customerAddress?`<div class="text-xs text-slate-400">${wo.customerAddress}</div>`:''}
        </div>
        <span class="status-pill ${statusColor(wo.status)}">${wo.status.replace(/_/g,' ')}</span>
      </div>
      ${wo.description?`<p class="text-xs text-slate-600 mt-1">${wo.description}</p>`:''}
      <div class="mt-2 flex flex-wrap gap-2 items-center">
        ${statusSelect(wo.status, wo.id)}
        <select class="assign-select border rounded-lg text-xs px-2 py-1.5" data-id="${wo.id}">
          <option value="">Reassign…</option>
          ${techs.map(t=>`<option value="${t.id}" ${t.id===wo.assignedTo?'selected':''}>${t.name}${t.van?' · '+t.van:''}</option>`).join('')}
        </select>
...
async function adminOrders() {
  ...
  const content = `
    <div class="space-y-1">
      <p class="text-xs text-slate-500">Jobs grouped by status. Tap 📅 Calendar to add a job to Google Calendar.</p>
